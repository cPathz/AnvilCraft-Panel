        ...
        >
            ...
            {:else if panelTab === 'format'}
                ...
                <div class="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-2">
                    {#if formatType === 'raw'}
                        <!-- Raw format - logs as-is from server -->
                        {#if logs.length > 0}
                            {#each logs.slice(-10) as log}
                                <div class="bg-black/30 rounded-lg p-2.5 border border-white/5">
                                    <div class="text-[11px] font-mono text-zinc-300 whitespace-pre-wrap break-words">
                                        {log}
                                    </div>
                                </div>
                            {/each}
                        {:else}
                            <div class="flex items-center justify-center py-8 opacity-30">
                                <span class="text-xs text-zinc-500">No hay logs</span>
                            </div>
                        {/if}
                    {:else if formatType === 'formato1'}
                        <!-- Formato 1 - parsed structured format -->
                        {#if logs.length > 0}
                            {#each logs.slice(-10) as log}
