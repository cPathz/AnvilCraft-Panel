use crate::loaders::mods::find_forge_args_file;
...
pub async fn start_instance(
    ...
    if let Some(stdout) = child.stdout.take() {
        ...
        // Proxy servers (BungeeCord, Waterfall, Velocity) signal readiness
        // with "Listening on /" instead of "Done (" — ask the registry
        // for the loader's is_proxy capability so any future proxy is
        // automatically covered without extra code here.
        let is_proxy = LoaderRegistry::global()
            .by_engine(instance_engine)
            .map(|l| l.capabilities().is_proxy)
            .unwrap_or(false);
            ...
            for line in reader.lines() {
                if let Ok(l) = line {
                    ...
                    if !is_running {
                        ...
                        let ready = if is_proxy {
                            // Proxy servers print "Listening on /0.0.0.0:XXXX" when ready.
                            lower.contains("listening on /")
                        } else {
                            lower.contains("done (") || lower.contains("for help, type \"help\"")
                        };
                        if ready {
                            is_running = true;
...
pub async fn stop_instance(
    ...
    // Determine stop command via the registry (proxies use "end", everything
    // else "stop"). Step 4: ask the registry instead of hardcoding a
    // Velocity/Waterfall match, so adding new proxy loaders (e.g. BungeeCord)
    // just works.
    let mut stop_cmd: &'static str = "stop";
            ...
            for entry in fs::read_dir(instances_dir).into_iter().flatten().flatten() {
                ...
                if json_path.exists() {
                    if let Ok(content) = fs::read_to_string(json_path) {
                        if let Ok(inst) = serde_json::from_str::<Instance>(&content) {
                            if inst.id == id {
                                stop_cmd = LoaderRegistry::global()
                                    .by_engine(inst.loader)
                                    .map(|l| l.stop_command())
                                    .unwrap_or("stop");
                                break;
                            }
                        }
