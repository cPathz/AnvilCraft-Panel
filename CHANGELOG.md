...
- **Forge 1.21.x server start detection** (`src-tauri/src/loaders/mods.rs::find_forge_args_file`): el installer Forge 1.21.x pone `win_args.txt` en `<mc_dir>/` directamente, no en `<mc_dir>/libraries/`. El detector solo buscaba recursivamente en `libraries/`, así que caía al "Classic launch mode" (`java -jar server.jar`) y el server no arrancaba. Ahora prueba `mc_dir/win_args.txt` primero y hace fallback al recursive search (cubre NeoForge y Forge ≤ 1.20.x).
- **Forge version listing from the loader strategy** (`src-tauri/src/loaders/mods.rs::ForgeLoader::fetch_versions`): antes devolvía `not_implemented_yet("Forge")`. Ahora delega a `get_project_versions("forge")` (que ya consulta `maven-metadata.xml`) y mapea a `VersionMeta` con la URL del installer y la MC version requerida. La UI puede pedir el dropdown de versiones de Forge vía el contrato estándar del loader strategy.
...
- **Validado en instancia real** (`dasdasd`, %APPDATA%/AnvilCraftPanel/instances/dasdasd/): Forge 1.21.11-61.1.14 instala y arranca correctamente. `run.bat` abre puerto 25565, genera world nuevo, "Done (3.204s)". El bug del `win_args.txt` arriba fue el que bloqueaba el flujo end-to-end desde AnvilCraft. Catalog `tested: true` ahora.
- **Errores cosméticos en log de Forge** (no bloqueantes): `sun.misc.Unsafe`, `jna` (`Use --enable-native-access=...`), `io.netty kqueue/epoll Native` (macOS/Linux only, ignorados en Windows), `Failed to hack DnsContextFactory` (solo afecta conexión a servers externos por nombre), `Configuration file ... is not correct. Correcting` (Forge creando defaults en primer arranque). Todos son **ruido normal** de un server modded moderno en Windows con Java 26.
